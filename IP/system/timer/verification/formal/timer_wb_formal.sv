// timer_wb_formal.sv — Flat formal wrapper for Wishbone B4 variant.
...
//
// Note: Wishbone uses active-high synchronous RST_I; rst_n = ~RST_I is derived
// internally. The formal wrapper uses CLK_I/RST_I as the primary clock/reset.
//
...
module timer_wb_formal #(
  ...
  input  wire [DATA_W-1:0]   DAT_I,
  input  wire [DATA_W/8-1:0] SEL_I,
  output wire [DATA_W-1:0]   DAT_O,
  output wire                ACK_O,
  output wire                ERR_O,
  output wire                irq,
  ...
  // Wishbone uses active-high synchronous reset; derive active-low for submodules
  wire rst_n = ~RST_I;
  ...
  // Bus interface
  claude_wb_if #(.DATA_W(DATA_W), .ADDR_W(ADDR_W)) u_if (
    .CLK_I   (CLK_I),
    .RST_I   (RST_I),
    .CYC_I   (CYC_I),
    .STB_I   (STB_I),
  ...
  // Note: WB reset is active-high synchronous, so we use RST_I.
  // reset_done tracks that reset has been seen at least once.
  // ---------------------------------------------------------------------------
  reg  reset_done;
  reg  past_rst_n;
  reg  past_ctrl_en;
  ...
  always @(posedge CLK_I) begin
    if ($initstate) assume (RST_I);
  end

  // Guard: only assert in a stable post-reset state
  wire check = reset_done && !RST_I && past_rst_n;
