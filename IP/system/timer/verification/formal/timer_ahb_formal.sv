// timer_ahb_formal.sv — Flat formal wrapper for AHB-Lite variant.
...
module timer_ahb_formal #(
  ...
  input  wire [DATA_W/8-1:0] HWSTRB,
  ...
  // Internal wires matching timer_ahb.sv
  wire                if_wr_en;
  ...
  always @(posedge HCLK) begin
    if ($initstate) assume (!HRESETn);
  end

  // Guard: only assert in a stable post-reset state
  wire check = reset_done && HRESETn && past_HRESETn;
