# create_project_ahb.tcl — Vivado project for timer AHB-Lite simulation
#
# Usage:
#   vivado -mode tcl -source create_project_ahb.tcl
#   or: source create_project_ahb.tcl   (from the Vivado Tcl console)
#
...
set work_dir  "${timer_dir}/verification/vivado/work/ahb"
set wave_tcl  "${script_dir}/wave_ahb.tcl"

# ---------------------------------------------------------------------------
# Create project
# ---------------------------------------------------------------------------
create_project tb_timer_ahb "${work_dir}" -part xc7z010clg400-1 -force
...
add_files -norecurse [list \
    ...
    "${common_dir}/design/rtl/verilog/claude_ahb_if.sv" \
    "${rtl_dir}/timer_ahb.sv"                          \
]
...
# ---------------------------------------------------------------------------
add_files -norecurse -sim_only "${tb_dir}/tb_timer_ahb.sv"
set_property file_type SystemVerilog [get_files tb_timer_ahb.sv]

# ---------------------------------------------------------------------------
# Simulation fileset configuration
# ---------------------------------------------------------------------------
set_property top              tb_timer_ahb       [get_filesets sim_1]
...
puts ""
puts "Project created: ${work_dir}/tb_timer_ahb.xpr"
puts "To simulate:     File > Open Project > work/ahb/tb_timer_ahb.xpr"
