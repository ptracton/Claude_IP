...
entity timer_wb is
  generic (
    ...
    RST_POL : natural  := 0   -- 0 = active-low (Wishbone RST_I is active-high)
  );
  port (
    -- Wishbone B4 signals
...
architecture rtl of timer_wb is
  ...
  -- Derive active-low reset for regfile and core from Wishbone active-high RST_I
...
begin
  ...
  -- Convert active-high RST_I to active-low for regfile/core
  rst_n <= not RST_I;

  -- Bus interface submodule
  u_wb_if : claude_wb_if
    generic map (
      DATA_W => DATA_W,
      ADDR_W => ADDR_W
    )
